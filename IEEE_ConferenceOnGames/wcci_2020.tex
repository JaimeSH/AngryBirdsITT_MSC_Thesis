...
and generates the next level to play. The model proposed by Stephenson and Renz \cite{DBLP:journals/corr/abs-1902-02518} also adapts itself to the level of difficult that's needed.
...
composite in the pool. A chromosome can include the same ID multiple times.
    ...
    %In Laura's work, we tried to avoid this because it took a long time. Is there any procedure to check constraints and not evaluate them? - JJ
    % Added a comment about this  - M 
    ...
    we mention before.  % Cite our paper, where we do exactly that - JJ 
                        % Done - Mario 
